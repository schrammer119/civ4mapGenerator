...
- [ ] Remove incomplete band-aid: delete `north_of_rivers` and `west_of_rivers` array initialization
- [ ] Update `_remove_river_segment()` to search and remove from `river_map` directly instead of tracking arrays
- [ ] Update validation logic to check `river_map` for existing segments instead of separate tracking
- [ ] Verify `place_validated_river_segment()` properly updates `river_map`
- [ ] Re-run test suite to confirm river generation still works
...
- [ ] Run `python test_planetsim.py` to confirm no AttributeError
- [ ] Verify rivers are generated and visible in output
- [ ] Check that no segmentation issues occur in test
...
## Phase 1: Resolve Code TODOs & Missing Implementations

### 1.1 Code TODO Resolution
...
## Phase 2: Complete Resource & Bonus Placement with Visualization

### 2.1 Finish Resource Placement Implementation
...
### 2.2 Add Test Visualization for Resources
...
## Phase 3: Project Structure & Tooling Cleanup
...
- [ ] Add patterns for build artifacts, __pycache__, .pyc files
