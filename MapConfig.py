...
class MapConfig:
    ...
    def _initialize_climate_parameters(self):
        ...
        # --- General Climate ---
        self.climateSmoothing = 4           # General smoothing radius for climate maps (temperature, moisture).
        ...
        self.currentSolverIterations = 100  # Max iterations for the ocean current solver. Higher is more accurate but slower.
        self.solverTolerance = 1e-1         # RMSE tolerance for the solver to converge. Lower is more accurate.
        self.minSolverIterations = 10       # Minimum iterations before checking for convergence.

        # --- Ocean Heat Transport ---
        self.max_plume_distance = 30        # Max distance a warm/cold water plume can travel from its source.
        self.mixing_factor = 0.95           # How much a plume retains its original temperature each step (0-1).
        self.min_strength_threshold = 0.01  # Minimum current strength required for a plume to continue flowing.
        self.current_amplification = 4.0    # Artificial multiplier to make current effects more pronounced for gameplay.
        ...
        self.maritime_strength = 0.7         # Strength of the ocean's temperature influence on coastal land (0-1).
        self.distance_decay = 0.5            # How quickly the maritime effect fades with distance from the coast.
        self.min_basin_size = 20             # Minimum size of a water body to have a maritime effect on adjacent land.

        # --- Wind ---
        self.horseLatitude = 30.0           # Latitude separating the Hadley and Ferrel atmospheric cells.
        self.polarFrontLatitude = 60.0      # Latitude separating the Ferrel and Polar atmospheric cells.
        self.tempGradientFactorWind = 0.2   # Strength of local winds generated by temperature gradients.
        self.orographicLiftFactor = 2.0     # How much wind is accelerated when flowing up mountains.
        self.valleyChannelingFactor = 1.5   # How much wind is accelerated when flowing through valleys.
        self.ridgeDeflectionDistance = 3    # How far away from a ridge wind patterns are deflected.
